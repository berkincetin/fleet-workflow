...
## Update (2026-08-15, task 8.5 close) — two findings above are now superseded

**1. CV extraction is no longer "not completed" — it is 100% (15/15) on this same CPU-only machine.** The summary table's "Not viable for a full clean batch on this machine" verdict was correct at the time but attributed to the wrong cause. Three concrete bugs, none of them hardware-related, were responsible for the CV lane's failures; all three are fixed in task 8.5:
- `evals/synthetic_images.py` clipped the 6th line of every document (fixed canvas height of 260px vs. a y-offset of 260 for line 6), so the CV fixtures' `Yetenekler:` line never reached OCR at all. Height is now derived from the line count. *This bug was introduced by this very rehearsal's own fix* — the renderer added here defaulted to a 5-line-sized canvas, fine for the 5-line invoices it was tested against, silently wrong for the 6+-line CVs it was built for.
- Tesseract reads a leading `+` in TR phone numbers as `*`; now normalised in `agents/hr_agent/extractor.py::_normalize_phone`.
- The extraction prompt's examples were English, so the model translated Turkish employers (`Fleet Lojistik` → `Fleet Logistics`); the prompt now mandates source-language transcription.

The recommendation to run CV evals only on GPU hardware is therefore **withdrawn**: `make eval AGENT=hr_agent` completes reliably on CPU in ~20 minutes. The `@pytest.mark.gpu` guidance still stands for *CI gating* (a 20-minute local-lane eval should not block a PR), but not for feasibility.

**2. The "litellm proxy timeouts under CPU load" issue is RESOLVED, and it was not a litellm or Ollama defect.** Measured layer by layer: raw Ollama `/api/generate` 4.3s; litellm proxy with a trivial prompt 4.7s; litellm proxy with the *real* CV extraction prompt 26–39s across repeated calls, all HTTP 200, zero errors. Nothing was intermittently failing. The failure was a **timeout mismatch between two layers**: task 8.1 raised `litellm_params.timeout` to 300s but the gateway *client*'s own HTTP timeout (`FLEET_LITELLM_TIMEOUT`, `apps/runtime/core/llm/factory.py`) stayed at its 60s default — close enough to the real 26–39s cost that batches tripped it. With `FLEET_LITELLM_TIMEOUT=300` the full 15-case eval runs end to end with no gateway errors. Documented in `.env.example`; the 60s default is deliberately kept so a genuinely stuck cloud call cannot hang for five minutes.

The remaining OPEN item from this report — invoice extraction never routing to the documented "Claude Sonnet on redacted text" — is unaffected and still awaits a decision.
